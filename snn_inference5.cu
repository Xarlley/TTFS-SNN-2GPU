...
// [FIX] 定义固定的积分常数 (来自 lib_snn.py 的默认配置)
// 原始代码中 integration 的 TC 和 TD 是不训练的，保持初始值
#define DEFAULT_TC_INTEG 20.0f
#define DEFAULT_TD_INTEG 0.0f
...
__global__ void k_layer_inference(
    ...
    float tc_fire,  // [当前层] 发放 TC (用于计算 Vth) -> 训练值
    float tc_integ, // [上一层] 积分 TC (用于积分衰减) -> 固定值 20.0
    float td_fire,  // [当前层] 发放 TD (用于计算 Vth) -> 训练值
    float td_integ, // [上一层] 积分 TD (用于积分补偿) -> 固定值 0.0
    bool is_fc                    // 是否为全连接层
) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
int main(int argc, char* argv[]) {
    ...
    std::cout << ">>> Starting SNN Inference (Corrected Logic)..." << std::endl;
    // 输入层的发放TC通常是被训练过的，保留用户从Python抓取的值
    // 如果你在Python端确认了其他值，请在此处修改
    float tc_in_fire = 17.452274f; 
    float td_in_fire = 0.0f;

    // Step A: Encode (输入层发放)
    k_encode_image<<<1, 784>>>(d_img, d_s0, 784, tc_in_fire, td_in_fire);
    CHECK_CUDA(cudaDeviceSynchronize());

    // Step B: Conv1
    // 注意：tc_integ 和 td_integ 必须是默认固定值 (20.0, 0.0)
    ...
    k_layer_inference<<<(threads_c1+255)/256, 256>>>(
        ...
        layers[0].tc_fire,    // 当前层(Conv1)的发放常数 (训练值)
        DEFAULT_TC_INTEG,     // 积分常数 (固定值 20.0) - [FIXED]
        layers[0].td,         // 当前层(Conv1)的延迟 (训练值)
        DEFAULT_TD_INTEG,     // 积分延迟 (固定值 0.0) - [FIXED]
        false
    );
    CHECK_CUDA(cudaDeviceSynchronize());
    ...
    k_layer_inference<<<(threads_c2+255)/256, 256>>>(
        ...
        12, 12, 12, 8, 8, 64, 5, 5,
        layers[1].tc_fire,    // 当前层(Conv2)的发放常数
        DEFAULT_TC_INTEG,     // 积分常数 (固定值 20.0) - [FIXED]
        layers[1].td,         // 当前层(Conv2)的延迟
        DEFAULT_TD_INTEG,     // 积分延迟 (固定值 0.0) - [FIXED]
        false
    );
    CHECK_CUDA(cudaDeviceSynchronize());
    ...
    k_layer_inference<<<1, 10>>>(
        ...
        layers[2].tc_fire,    // 当前层(FC1)的发放常数
        DEFAULT_TC_INTEG,     // 积分常数 (固定值 20.0) - [FIXED]
        layers[2].td,         // 当前层(FC1)的延迟
        DEFAULT_TD_INTEG,     // 积分延迟 (固定值 0.0) - [FIXED]
        true
    );
    CHECK_CUDA(cudaDeviceSynchronize());
